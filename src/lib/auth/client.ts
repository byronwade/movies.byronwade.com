...
function storageGet(): string | null {
  try {
    ...
    if (session) {
      window.localStorage.setItem(BEARER_KEY, session);
      return session;
    }
  } catch {
    /* ignore */
...
function storageSet(token: string | null) {
  try {
    if (token) {
...
export function captureSessionToken(token: string | null): void {
...
/** Popup only when the preview is actually framed. Top-level in-app browsers (iPhone Grok) cannot keep a popup alive — they freeze on wait. */
function usePopupSignIn(): boolean {
  return inLivePreview() && inIframe();
}
...
function waitForPopupToken(popup: Window): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    const pollTimer = window.setInterval(() => {
      ...
      closeTimer = window.setTimeout(() => settle(null), 1200);
    }, 300);
    const hangTimer = window.setTimeout(() => settle(null), 45_000);
    function cleanup() {
      window.clearInterval(pollTimer);
