...
import { publicGateSigninUrl } from "./login.ts";
import {
...
function connectorsBaseFor(publicHost: string | null): string | null {
  ...
  // Live preview is served on *.grok-sandbox.com; connector calls still go
  // through the production connectors host with the injected viewer token.
...
const previewCred = globalThis as typeof globalThis & {
  __grokAppDataPreviewToken?: string;
  __grokAppDataPreviewHost?: string;
};
...
function inboundContext(): InboundContext {
  ...
  const origin =
    hostnameOf(req?.headers.get("origin")) ?? hostnameOf(req?.headers.get("referer"));
  ...
  // Single-owner live preview: document GETs carry the connector token, while
  // later server-fn POSTs sometimes do not. Remember the last non-loopback
  // creds for this process only — never in production (multi-tenant).
  if (process.env.NODE_ENV !== "production") {
    if (headerToken) previewCred.__grokAppDataPreviewToken = headerToken;
    if (publicHost && !isLoopbackHost(publicHost)) previewCred.__grokAppDataPreviewHost = publicHost;
    if ((!publicHost || isLoopbackHost(publicHost)) && previewCred.__grokAppDataPreviewHost) {
      publicHost = previewCred.__grokAppDataPreviewHost;
    }
  }
  ...
  return {
    token:
      headerToken ??
      envToken ??
      (process.env.NODE_ENV === "production" ? null : (previewCred.__grokAppDataPreviewToken ?? null)),
    publicHost,
...
export function rememberInboundConnector() {
...
function gateSigninUrl(ctx: InboundContext): string | undefined {
  ...
  const returnTo = gated ? `${gated}/profile` : undefined;
  // The live-preview app origin does not serve /__gate/signin (it 404s as a
  // TanStack route). Always send sandbox viewers to the real gate.
  if (!publicHost || isLoopbackHost(publicHost) || publicHost.endsWith(".grok-sandbox.com")) {
    return publicGateSigninUrl(returnTo);
  }
  if (gated) {
    return `${gated}/__gate/signin?return_to=${encodeURIComponent(returnTo ?? gated)}`;
  }
  const base = ctx.connectorsBase;
  if (!base) return publicGateSigninUrl(returnTo);
  try {
    const connectorsHost = new URL(base).host.toLowerCase();
    const gateHost = connectorsHost.replace(/^connectors\./, "gate.");
    if (gateHost === connectorsHost) return publicGateSigninUrl(returnTo);
    const signin = `https://${gateHost}/__gate/signin`;
    return returnTo ? `${signin}?return_to=${encodeURIComponent(returnTo)}` : signin;
  } catch {
    return publicGateSigninUrl(returnTo);
  }
}
