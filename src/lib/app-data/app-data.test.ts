...
import { isLoginRequired, redirectToLoginIfRequired, connectorLoginUrl } from "./login.ts";
...
describe("connectorLoginUrl", () => {
  it("uses the real Grok gate on a live-preview host", () => {
    const url = withWindow(
      {
        location: {
          assign: () => undefined,
          href: "https://kino-preview.grok-sandbox.com/profile",
        },
      },
      () => connectorLoginUrl(),
    );
    assert.equal(
      url,
      "https://gate.grok.me/__gate/signin?return_to=https%3A%2F%2Fkino-preview.grok-sandbox.com%2Fprofile",
    );
  });

  it("rewrites a same-origin preview /__gate/signin URL", () => {
    assert.equal(
      connectorLoginUrl(
        "https://kino-preview.grok-sandbox.com/__gate/signin?return_to=https%3A%2F%2Fkino-preview.grok-sandbox.com%2Fprofile",
      ),
      "https://gate.grok.me/__gate/signin?return_to=https%3A%2F%2Fkino-preview.grok-sandbox.com%2Fprofile",
    );
  });

  it("keeps an explicit server-built loginUrl", () => {
    assert.equal(
      connectorLoginUrl("https://gate.grok.me/__gate/signin?return_to=x"),
      "https://gate.grok.me/__gate/signin?return_to=x",
    );
  });
});
...
describe("redirectToLoginIfRequired", () => {
  ...
  it("builds a gate URL from the current page when loginUrl is missing", () => {
    let target = "";
    const did = withWindow(
      {
        location: {
    ...
    assert.equal(
      target,
      "https://my-app.grok.me/__gate/signin?return_to=https%3A%2F%2Fmy-app.grok.me%2Fprofile",
    );
  });
